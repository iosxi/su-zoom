...
単眼鏡は双眼鏡の 1 本ぶん、つまり筒であって、虫眼鏡でも片眼鏡でもない。
接眼・胴・対物の 3 つを角丸の長方形で描き、45 度に寝かせている。丸い端どうし
を継ぐと骨のような形になるので、円ではなく角丸の長方形を使う。16px では
対物のレンズとピントリングに加えて接眼のふくらみも省く。両端が太いと中央が
くびれて、やはり骨に見えるため。

アイコンは「地の色 + 枠」を必ず持たせること。背景を持たない黒いグリフは
ツールバーの配色によっては沈んで見えなくなる。これを避けるのが今回の
要件のひとつ。
...
### 「ユーザーのコンピューター上のローカルファイルへのアクセス」は要求していない

Firefox 153 から、`file://` へのアクセスが独立した任意の権限になった。
`about:addons` の「権限とデータ」に切り替えとして出る。

この行が出る条件は Firefox 側にあり (`Extension.sys.mjs` の
`formatPermissionStrings`)、次のどれかを満たすときだけ:

1. 必須または任意のホスト権限に `<all_urls>` か `file://…` が入っている
2. `internal:fileSchemeAllowed` が **すでにその環境で許可済み**

su-zoom はホスト権限を 1 つも持たないので 1 には当たらない。インストール時の
確認画面には出ない (`ExtensionsUI.sys.mjs` は `includeFileSchemeAccess` を
渡していない)。

それでも切り替えが見えるなら 2 で、`<all_urls>` を持っていた 2.0.0 までの
許可がプロファイルに残っている。Firefox は「あとから切れるように」と、
マニフェストの内容に関わらずこの行を出す。切り替えを **オフにすれば消える**。
拡張を削除して入れ直しても同じ。マニフェスト側で消せるものではない。
